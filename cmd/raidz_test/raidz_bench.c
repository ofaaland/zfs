...
#define	REC_BENCH_MEMORY	(((uint64_t)1ULL)<<30)
...
{
	IPRINT(DBLSEP "Initializing benchmark data ...\n\n");
...
static inline void
__run_gen_bench(const char *impl)
{
	int fn, ncols;
...
run_gen_bench(void)
{
	...
	IPRINT(DBLSEP "Benchmarking parity generation...\n\n");
	...
	for (impl_name = (char **)raidz_impl_names; *impl_name != NULL;
		...
		__run_gen_bench(*impl_name);
	}
}

static void
__run_rec_bench(const char *impl)
{
	struct rusage start, stop;
	int fn, ncols, nbad;
	uint64_t ds, iter_cnt, iter, disksize;
...
run_rec_bench(void)
{
	...
	IPRINT(DBLSEP "Benchmarking data reconstruction...\n\n");
...
run_raidz_benchmark(void)
{
	...
	PRINT("impl, math, dcols, iosize, disk_bw, total_bw, iter\n");
