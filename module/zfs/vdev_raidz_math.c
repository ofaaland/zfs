...
static enum vdev_raidz_impl_sel {
	 ...
	 */
} zfs_vdev_raidz_impl = IMPL_FASTEST;
...
raidz_math_ops_t *raidz_supp_maths[ARRAY_SIZE(raidz_all_maths)] = {
	(raidz_math_ops_t *)&vdev_raidz_scalar_impl /* scalar is supported */
};

/* Holding kstats for all algos + original methods */
static raidz_math_ops_kstat_t raidz_math_kstats[ARRAY_SIZE(raidz_all_maths)+1];
...
{
	const int impl_num = ARRAY_SIZE(raidz_all_maths);
...
zfs_vdev_raidz_impl_set(const char *val, struct kernel_param *kp)
{
	...
	/* Allow whitespace et the end of the argument */
	val_len = strnlen(val, 32);
	while ((val_len > 0) && !!isspace(val[val_len-1]))
		val_len--;

	/* Check if the passed value is valid */
	for (idx = 0; idx < ARRAY_SIZE(math_impl_opt_names); idx++) {
		len = strlen(math_impl_opt_names[idx]);
		if ((len == val_len) &&
		    (0 == strncmp(val, math_impl_opt_names[idx], len))) {
			i = idx;
			break;
		}
	}
	...
	/* Invalid value */
	if (idx >= ARRAY_SIZE(math_impl_opt_names))
		return (-EINVAL);

	impl = vdev_raidz_used_impl;


	/* Select requested implementation, if supported */
	if (idx == 0) {
		impl = &vdev_raidz_fastest_impl;
		zfs_vdev_raidz_impl = IMPL_FASTEST;
	} else if (idx == 1) {
		impl = NULL;
		zfs_vdev_raidz_impl = IMPL_ORIGINAL;
	} else if (idx == 2) {
		impl = NULL;
		zfs_vdev_raidz_impl = IMPL_CYCLE;
	} else {
		/*
		 * Select specific implementation:
		 * Check whether an implementation is supported on the
		 * current system. If this code path is taken before module
		 * init, only scalar impl. will be available (can always run).
		 * Other, vector implementations will be selectable when module
		 * is finished loading.
		 */
		err = -EINVAL;
		for (idx = 0; idx < raidz_supp_maths_cnt; idx++) {
			len = strlen(raidz_supp_maths[idx]->name);
			if ((len == val_len) &&
			    (0 == strncmp(val, raidz_supp_maths[idx]->name,
			    len))) {
				impl = raidz_supp_maths[idx];
				zfs_vdev_raidz_impl = idx;
				err = 0;
				break;
			}
		}
...
zfs_vdev_raidz_impl_get(char *buffer, struct kernel_param *kp)
{
	...
	cnt += sprintf(buffer + cnt,
	    (zfs_vdev_raidz_impl == IMPL_FASTEST) ? "[%s] " : "%s ",
	    "fastest");
	cnt += sprintf(buffer + cnt,
	    (zfs_vdev_raidz_impl == IMPL_ORIGINAL) ? "[%s] " : "%s ",
	    "original");
	cnt += sprintf(buffer + cnt,
	    (zfs_vdev_raidz_impl == IMPL_CYCLE) ? "[%s] " : "%s ",
	    "cycle");

	for (i = 0; i < raidz_supp_maths_cnt; i++) {
		cnt += sprintf(buffer + cnt,
		    (zfs_vdev_raidz_impl == i) ? "[%s] " : "%s ",
